...
process LoopLearnErrors {
    ...
    output:
    file "errors.RDS" into errorsLoop
    file "*.pdf"
...
process LoopPoolSamplesInferDerep {
...
process GenerateSeqTables {
    ...
    # FIX ME - THE SAMPLE NAMES ARE FILENAMES, BUT NEED TO BE SAMPLE IDS
    # FOR COMPATIBILITY WITH METADATA FILES
    write.table(
...
*    rownames(dadas) <- gsub('.*_sample_', '',rownames(dadas))
*    rownames(dadas) <- gsub('_contig_list.*', '',rownames(dadas))
*    dadas\$SampleID <- rownames(dadas)
*
*    seqtab.nochim <- as.data.frame(rowSums(readRDS("${sTable}")))
*    rownames(seqtab.nochim) <- gsub('.*_sample_', '',rownames(seqtab.nochim))
*    rownames(seqtab.nochim) <- gsub('_contig_list.*', '',rownames(seqtab.nochim))
*    seqtab.nochim\$SampleID <- rownames(seqtab.nochim)
*
*    trimmed <- read.csv("${trimmedTable}")
*    rownames(trimmed) <- gsub('.*_sample_', '',trimmed\$Sequence)
*    rownames(trimmed) <- gsub('_contig_list.*', '', rownames(trimmed))
